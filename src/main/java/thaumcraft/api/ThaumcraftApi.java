...
public class ThaumcraftApi {
    ...
    public static Object[] getCraftingRecipeKey(EntityPlayer player, ItemStack stack) {
        ...
        if (keyCache.containsKey(key)) {
            ...
            if (ThaumcraftApiHelper.isResearchComplete(player.getCommandSenderName(), (String) (keyCache.get(key))[0])) return keyCache.get(key);
            else return null;
        }
                    ...
                    if (page.recipe != null && page.recipe instanceof CrucibleRecipe[]) {
                        ...
                        for (CrucibleRecipe cr : crs) {
                            if (cr.getRecipeOutput().isItemEqual(stack)) {
                                ...
                                if (ThaumcraftApiHelper.isResearchComplete(player.getCommandSenderName(), ri.key)) return new Object[] { ri.key, a };
                            }
                        }
     ...
     * 
     * @param item,
     *            pass OreDictionary.WILDCARD_VALUE to meta if all damage values
     *            of this item/block should have the same aspects
     * @param aspects
     *            A ObjectTags object of the associated aspects
     */
    ...
    public static void addLootBagItem(ItemStack item, int weight, int... bagTypes) {
        if (bagTypes == null || bagTypes.length == 0) WeightedRandomLoot.lootBagCommon.add(new WeightedRandomLoot(item, weight));
        else {
